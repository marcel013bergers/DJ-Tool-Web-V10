...
def render_quick_backup_button():
    ...
    backup_key = f"quick_backup_bytes::{get_runtime_data_signature()}::{APP_SHORT_VERSION}"
    if st.button("📦 Backup vorbereiten", key="prepare_quick_backup_btn", width="stretch"):
        try:
            ...
            st.session_state["quick_backup_ready_key"] = backup_key
            st.success("Backup ist bereit zum Download.")
        except Exception as e:
            st.warning(f"Backup aktuell nicht verfügbar: {e}")

    ready_key = st.session_state.get("quick_backup_ready_key")
    ready_bytes = st.session_state.get(ready_key) if ready_key else None
    if ready_bytes:
        st.download_button(
...
BUILD_NOTES = [
    "V152 macht Backup-Download lazy und bremst das UI dadurch weniger aus",
    "V152 ergänzt DB-signaturbasierte Caches für Listen, Tracks und Analysen",
    "V152 ergänzt SQLite-Indexe für Playlists und Track-Reihenfolge",
...
CHANGELOG = [
    {"version": "V152", "date": "2026-04-04", "new": ["Lazy Backup Download", "DB-Signatur-Caches", "SQLite-Indexe"], "fixes": ["kein schweres ZIP mehr bei jedem Render", "schnellere Filter/Listen", "ruhigere Streamlit-Reruns"]},
...
def _compute_data_cached(event=None, source=None, top_only=False, sub_event=None, data_signature: str = ""):
    del data_signature
...
def compute_data_cached(event=None, source=None, top_only=False, sub_event=None):
    return _compute_data_cached(event=event, source=source, top_only=top_only, sub_event=sub_event, data_signature=get_runtime_data_signature())

def clear_runtime_caches():
    for fn in (_compute_data_cached, _get_all_playlist_sequences_cached, _get_playlist_tracks_cached, _get_playlists_cached, _get_distinct_values_cached, _get_distinct_sub_events_cached, _get_recent_import_runs_cached):
        try:
            fn.clear()
        except Exception:
            pass
...
def render_context_help_box(menu_name: str):
    ...
    if not cfg:
        return
    with st.expander(f"ℹ️ Hilfe zu: {menu_name}", expanded=menu_name in {"Event Presets", "Auto Event Set", "Analyse Hub", "Learning Dashboard", "Live Hilfe", "Playlists importieren", "Set Builder", "Event vorbereiten"}):
...
def render_learning_dashboard_page():
    ...
    m4.metric("Confidence", pack["confidence_label"])
    m5.metric("Playlists", pack["playlist_count"])

    st.caption(pack["basis_text"])
    st.info(" | ".join(pack["explanation"]))
    if pack.get("weak_signals"):
        with st.expander("⚠️ Dünn gelernte Signale", expanded=False):
            for item in pack["weak_signals"]:
                st.write(f"- {item}")
...
def get_merged_distinct_values(column_name, presets=None):
    ...
    merged = ["Alle"]
    seen = {"alle"}
    for value in list(presets) + list(values):
...
def render_backup_restore_page():
    ...
    try:
        backup_bytes = build_full_backup_zip_bytes()
        st.download_button(
            label="📦 Backup jetzt herunterladen",
            data=backup_bytes,
            file_name=f"dj_tool_backup_{APP_BUILD_DATE}_{APP_BUILD_TIME.replace(':', '-')}.zip",
            mime="application/zip",
            width="stretch",
...
def get_runtime_data_signature() -> str:
    parts = []
    for raw_path in (DB_PATH, SET_FILE):
        try:
            p = Path(raw_path)
            stat = p.stat()
            parts.append(f"{p.name}:{int(stat.st_mtime_ns)}:{stat.st_size}")
        except Exception:
            parts.append(f"{Path(raw_path).name}:missing")
    return "|".join(parts)
...
def init_db():
    ...
    cur.execute("CREATE INDEX IF NOT EXISTS idx_playlists_event ON playlists(event)")
    cur.execute("CREATE INDEX IF NOT EXISTS idx_playlists_sub_event ON playlists(sub_event)")
    cur.execute("CREATE INDEX IF NOT EXISTS idx_playlists_source ON playlists(source)")
    cur.execute("CREATE INDEX IF NOT EXISTS idx_playlists_is_top ON playlists(is_top)")
...
def _get_recent_import_runs_cached(limit: int, data_signature: str):
    del data_signature
    conn = get_conn()
    try:
        cur = conn.cursor()
...
def get_recent_import_runs(limit: int = 20):
    return _get_recent_import_runs_cached(int(limit), get_runtime_data_signature())
...
def get_learning_engine_status():
    ...
    is_current = bool(meta.get("last_rebuild_at")) and basis_playlists == playlists and basis_tracks == tracks
    return {
...
def render_learning_engine_panel():
    ...
    if status["is_current"]:
        ...
        st.warning("Learning Engine ist veraltet. Neue oder geänderte Playlists wurden erkannt. Bitte Rebuild starten.")
    ...
    try:
        ...
        if current_playlists > 0 and current_tracks > 0 and (transition_rows <= 0 or role_rows <= 0):
            ...
            st.session_state["learning_auto_rebuild_status"] = (
                f"Learning wurde beim Start automatisch wiederhergestellt: "
                f"{result['transitions']} Übergänge / {result['roles']} Rollen."
            )
        else:
            st.session_state["learning_auto_rebuild_status"] = ""
...
def _get_distinct_sub_events_cached(event_value: str, data_signature: str):
    del data_signature
    ...
    return ["Alle"] + sorted(dedup.values(), key=sort_key)


def get_distinct_sub_events(event_value: str = ""):
    return _get_distinct_sub_events_cached(event_value, get_runtime_data_signature())
...
@st.cache_data(ttl=30, show_spinner=False)
def _get_distinct_values_cached(column_name, data_signature: str):
    del data_signature
    conn = get_conn()
    try:
        cur = conn.cursor()
    ...
    return ["Alle"] + sorted(dedup.values(), key=lambda x: x.casefold())


def get_distinct_values(column_name):
    return _get_distinct_values_cached(column_name, get_runtime_data_signature())
...
def get_playlists(event=None, source=None, sub_event=None, top_only=False, genre=None):
    return _get_playlists_cached(event=event, source=source, sub_event=sub_event, top_only=top_only, genre=genre, data_signature=get_runtime_data_signature())


@st.cache_data(ttl=20, show_spinner=False)
def _get_playlist_tracks_cached(playlist_id, data_signature: str):
    del data_signature
    conn = get_conn()
    try:
        cur = conn.cursor()
...
def get_playlist_tracks(playlist_id):
    return _get_playlist_tracks_cached(playlist_id, get_runtime_data_signature())
...
def get_track_insights(query_track, event=None, source=None, top_only=False, sub_event=None):
    ...
    results = compute_data(event=event, source=source, top_only=top_only, sub_event=sub_event)
...
@st.cache_data(ttl=30, show_spinner=False)
def _get_all_playlist_sequences_cached(data_signature: str):
    del data_signature
    conn = get_conn()
    try:
        cur = conn.cursor()
...
def get_all_playlist_sequences():
    return _get_all_playlist_sequences_cached(get_runtime_data_signature())
...
def render_system_version_page():
    ...
    st.info("V9.8 bündelt Bereichs-Hilfe, Learning Dashboard im Menü und klarere Lern-Erklärungen. Ziel: echte Playlist-Lernlogik sichtbarer und verständlicher machen.")
...
def render_transition_learning_page():
    ...
    m2.metric("Direkte Folgetracks", len(pack.get("suggestions", [])))
    m3.metric("DJ-Ketten", len(chain_pack.get("chain_rows", [])))

    tabs = st.tabs(["🎯 Beste Folgetracks", "🔗 DJ-Ketten", "🧱 3er/4er/5er Wege"])

    with tabs[0]:
        if pack.get("suggestions"):
            for idx, row in enumerate(pack["suggestions"], start=1):
                cols = st.columns([4.6, 0.9, 1.0, 1.0])
                cols[0].write(f"{idx}. {row['song']}")
                cols[1].write(f"{row['transition_count']}x")
                cols[2].caption(row["role"])
                if cols[3].button("➕", key=f"transition_song_add_{idx}"):
                    st.session_state.setdefault("set_builder", []).append(row["song"])
                    st.success("Song ins Set übernommen.")
                    st.rerun()
                st.caption(f"{row['phase']} | {row['timing']}")
...
def render_login_gate():
    ...
    st.title("🔒 DJ Tool Login")
    st.caption("Dieses Tool ist geschützt. Nur freigegebene Personen kommen hinein.")
    ...
    persistent_token = get_persistent_login_token()
    if get_auth_query_value() == persistent_token:
        st.session_state["is_authenticated"] = True
    ...
    with st.container(border=True):
        ...
        pw = st.text_input("Passwort", type="password", key="login_password_input")
        keep_login = st.checkbox("Auf diesem Gerät eingeloggt bleiben", value=True, key="keep_login_checkbox")
        if st.button("Einloggen", type="primary", width="stretch"):
            if str(pw or "") == str(APP_LOGIN_PASSWORD):
                st.session_state["is_authenticated"] = True
                if keep_login:
        ...
        st.info("Wenn der Haken aktiv ist, merkt sich dein Browser den Login jetzt robuster über Gerätespeicher + Cookie.")
    return False

# ---------- UI ----------
st.set_page_config(page_title=f"DJ Tool {APP_SHORT_VERSION}", layout="wide")
init_db()
maybe_restore_from_supabase_on_startup()
maybe_restore_from_dropbox_on_startup()
persist_storage_info_snapshot()
apply_ipad_optimized_css()
ensure_learning_engine_safe_on_startup()
...
render_cloud_safe_info()
render_workflow_tip_box()
...
with st.sidebar:
    ...
    if chosen_menu != st.session_state["active_menu"]:
        st.session_state["active_menu"] = chosen_menu
    ...
    st.caption(f"Baseline: {APP_BASELINE_ID}")
...
if menu == "Event-Phasen":
    ...
    top_depth = f3.selectbox("Tiefe", [10, 20, 50, 100], index=1, key="hub_depth")
    ...
    data = compute_data(event=filter_event, source=filter_source, top_only=top_only, sub_event=filter_sub_event)
    ...
    with tabs[0]:
        ...
        rows = list(data["track_total_counts"].most_common(top_depth))
        if not rows:
    ...
    with tabs[1]:
        ...
        rows = list(data["pair_counts"].most_common(top_depth))
        if not rows:
    ...
    with tabs[2]:
        ...
        if block_mode == "3er-Blöcke":
            block_rows = list(data["block3_counts"].most_common(min(top_depth, 25)))
            combo_type = "3er"
            empty_text = "Keine 3er-Blöcke gefunden."
            key_prefix = "hub_b3"
        elif block_mode == "4er-Blöcke":
            block_rows = list(data["block4_counts"].most_common(min(top_depth, 25)))
            combo_type = "4er"
            empty_text = "Keine 4er-Blöcke gefunden."
            key_prefix = "hub_b4"
        else:
            block_rows = list(data["block5_counts"].most_common(min(top_depth, 25)))
            combo_type = "5er"
    ...
    if live_track:
        ...
        if not insights or not current:
            ...
            st.info(f"Typischer Einsatz: {current['timing']} | Insgesamt gesehen: {insights['total_count']}x")
